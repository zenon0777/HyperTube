...
export default function Home() {
  ...
  useEffect(() => {
    const getTMDBFiltredMovies = async () => {
      try {
        let baseUrl = `http://0.0.0.0:9000/movies/tmdb_movie_list`;
    ...
    const getTrendingTvShows = async () => {
      try {
        let baseUrl = `http://0.0.0.0:9000/movies/trending_tv_shows?&language=en-US&page=1`;
    ...
    const getYTSFiltredMovies = async () => {
      try {
        let baseUrl = `http://0.0.0.0:9000/movies/yts_movie_list?page=1&limit=15`;
